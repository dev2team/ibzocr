...
export default class Main extends Vue implements ControlInterface {
     ...
     * @memberof Main
     */
    private url: string = 'ocrweb/ocr/ocrrecord/maingrid/';
    ...
    public allColumns: any[] = [
        {
            name: 'ocrrecordid',
            label: '识别记录标识',
            langtag: 'ocrrecord.main_grid.columns.ocrrecordid',
            show: true,
            util: 'PX'
        },
        {
            name: 'ocrrecordname',
            label: '识别记录名称',
            langtag: 'ocrrecord.main_grid.columns.ocrrecordname',
            show: true,
            util: 'PX'
        },
        {
            name: 'procstate',
            label: '处理状态',
            langtag: 'ocrrecord.main_grid.columns.procstate',
            show: true,
            util: 'px'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'ocrrecord.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ocrrecord_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ocrrecord_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            <div class='grid'>
                <el-table  
                    ...
                    ref='multipleTable' data={this.items} show-header={!this.isHideHeader}>
                        ...
                        { this.renderocrrecordid(content) }
                        { this.renderocrrecordname(content) }
                        { this.renderprocstate(content) }
                        { this.renderupdatedate(content) }
     ...
     * @memberof Main
     */
    public renderocrrecordid(content: any) {
        let column = this.allColumns.find((col: any) => Object.is('ocrrecordid', col.name));
        if(column.show) {  
            return (
                <el-table-column show-overflow-tooltip prop='ocrrecordid' label={this.$t('ocrrecord.main_grid.columns.ocrrecordid')} width="250" align='left' sortable='custom' {...content}>
                </el-table-column>
            );
        }
    }
     ...
     * @memberof Main
     */
    public renderocrrecordname(content: any) {
        let column = this.allColumns.find((col: any) => Object.is('ocrrecordname', col.name));
        if(column.show) {  
            return (
                <el-table-column show-overflow-tooltip prop='ocrrecordname' label={this.$t('ocrrecord.main_grid.columns.ocrrecordname')} width="250" align='left' sortable='custom' {...content}>
                </el-table-column>
            );
        }
    }
     ...
     * @memberof Main
     */
    public renderprocstate(content: any) {
        let column = this.allColumns.find((col: any) => Object.is('procstate', col.name));
        if(column.show) {  
            return (
                <el-table-column show-overflow-tooltip prop='procstate' label={this.$t('ocrrecord.main_grid.columns.procstate')} width="150" align='left' sortable='custom' {...content}>
                </el-table-column>
            );
        }
    }
     ...
     * @memberof Main
     */
    public renderupdatedate(content: any) {
        let column = this.allColumns.find((col: any) => Object.is('updatedate', col.name));
        if(column.show) {  
            return (
                <el-table-column show-overflow-tooltip prop='updatedate' label={this.$t('ocrrecord.main_grid.columns.updatedate')} width="250" align='left' sortable='custom' {...content}>
                </el-table-column>
            );
        }
    }
